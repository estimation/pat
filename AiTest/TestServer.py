...
def AppFunction(environ, start_response):
    ...
    if method == "POST" and path == "/test":
        ...
        with file(tmpPath, 'w') as fp:
            fp.writelines(lines)
            # fp.write(s)

        if CONFIG['pre_planbatchid'] == "":
            print "本次执行判定为:首次执行; 执行计划号:".decode('utf-8').encode('gbk') + CONFIG['plan_id'] + ", 组号:".decode(
                'utf-8').encode('gbk') + CONFIG['group_id'] + ", 请关注执行情况".decode('utf-8').encode('gbk')
        else:
            print "本次执行判定为:失败重执行执行; 重执行计划批次号:".decode('utf-8').encode('gbk') + CONFIG[
                'pre_planbatchid'] + ", 组号:".decode('utf-8').encode('gbk') + CONFIG['group_id'] + ", 请关注重执行情况".decode(
                'utf-8').encode('gbk')
    ...
    # get指令
        ...
        if path[0:8] == "/restart":
            ...
            start_response('200 OK', [('Content-Type', 'application/force-download'),
                                      ('Content-Disposition', 'attachment;filename=selenium.log')])
