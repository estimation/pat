...
def csInput(inObj, checkType, inTxt, inActionMsg):
    ...
    inputVal=inTxt
    # 步骤输出结束
    StepReport.AI_PrintAtStepEnd_E("成功输入[" + inputVal + "]")
...
def moveToElement(inObj, checkType, inActionMsg):
    '''
    :param inObj:
    :param checkType:
    :param inActionMsg:
    ...
    '''
    # 步骤输出开始
    StepReport.AI_PrintAtStepBegin("鼠标悬停到[" + inActionMsg + "]")

    moveToElement_v(inObj)
    # 步骤输出结束
    StepReport.AI_PrintAtStepEnd_E("成功悬停在[" + inActionMsg  + "]")
...
def getAlertText(inActionMsg, InV=[]):
    """
    /**
     * 返回对象指定的属性值
     *
    ...
    if (inActionMsg != "NULL" and inActionMsg !=""):
        ...
        vCfg = StepReport.AI_GetStepConfig(inActionMsg)
        # 步骤输出开始
        StepReport.AI_PrintAtStepBegin("获取[" + vCfg["stepname"]+"提示框属性值")
        #执行
        attrVal=getAlertText_v(Inv)
        #步骤输出结束
        StepReport.AI_PrintAtStepEnd_E("获取提示框信息值为[" + attrVal + "]")
        #将值加入到输出列表中
        #DataPool.AI_SetOutputData(vCfg.get("outputkey"), attrVal, Integer.parseInt(vCfg.get("group")))
    else:
        attrVal=getAlertText_v(InV)
...
def contextClick(inObj, checkType, inActionMsg):
    # 步骤输出开始
    StepReport.AI_PrintAtStepBegin("右击[" + inActionMsg + "]")

    contextClick_v(inObj)
    # 步骤输出结束
    StepReport.AI_PrintAtStepEnd_E("成功右击[" + inActionMsg  + "]")
...
def inputAlertText(inTxt, inActionMsg):
    """
    WEB操作，提示框输入
    :param inTxt:输入内容
    :param inActionMsg:步骤信息
    ...
    vCfg = StepReport.AI_GetStepConfig(inActionMsg)
    # 步骤输出开始
    StepReport.AI_PrintAtStepBegin("在[" + vCfg["stepname"] + "]提示框中输入[" + inTxt + "]")
    # self.RegistObservers()
    #self.setSubjectState(inObj)  # 对象
    # Notify()
    inputAlertText_v(inTxt)
    # inputVal = csInput(inObj, inTxt)
    inputVal=inTxt
    # 步骤输出结束
    StepReport.AI_PrintAtStepEnd_E("成功输入提示框信息[" + inputVal + "]")
