...
class ExecuteDb(object):
    ...
    def __init__(self):
        ...
        sql_0 = "select project_id,eparchy_code from tbl_plan_def where plan_id='" + self.plan_id + "'"
        ID = self.DB.getSqlList(sql_0)
        ...
        self.env = '1'
        # 对执行状态初始化（考虑异常退出的情况）
        sql_1 = "select plan_batch_id from tbl_plan_exec where plan_id='" + self.plan_id + "' and exec_index='1' and status in ('1','2')"
        if self.DB.getSqlFirst(sql_1) != '':
            end_time = self.getCurrentTime()
            sql = "update tbl_plan_exec set status= 4,end_time='" + end_time + "' where plan_batch_id='" + plan_batch_id + "'"
            self.DB.dbwSql(sql)
    ...
    def get_PlanBatchId(self):
        ...
        logging.info("Now plan_batch_id is: "+plan_batch_id)
    ...
    def plan_Import_Cases(self, casespath, path):
        if self.group_id == '' and self.pre_planbatchid == '':
            ...
            #     logging.info(
            #         "当前计划".decode('utf-8').encode('gbk') + self.plan_id + "，正在执行中，不允许同时执行".decode('utf-8').encode(
            #             'gbk'))
            #     raise SystemExit
            # else:
            #     sql = "select case_id from tbl_plan_case_rel where plan_id='" + self.plan_id + "' and nodefunc_type='CASE' and status='1'"
            ...
            # if self.DB.getSqlFirst(tmpsql) != '0':
            #     logging.warning("当前计划".decode('utf-8').encode('gbk') + self.plan_id + "对应的".decode('utf-8').encode(
            #         'gbk') + self.group_id + "号组计划，正在执行中，不允许同时执行".decode('utf-8').encode('gbk'))
            #     raise SystemExit
            if self.pre_planbatchid == '':
        ...
        tmpsql = "select C.script_name from tbl_fk_case_script_rel A, tbl_fk_scripts C where A.project_id ='" + self.project_id + "' and A.case_id in (" + sql + ") and A.script_id=C.script_id"
        case_id = self.DB.getSqlList(tmpsql)
        # print case_id
        logging.info("Total Scripts :"+str('||'.join([x[0] for x in case_id])))
        ...
        for caseid in case_id:
            ...
            # print caseid
            logging.info("The " + str([x[0] for x in case_id].index(caseid) + 1) + "th script : " + str(caseid))
            try:
                tag = True
                ...
                # tag = True
                # with file(casespath + caseid,'r+') as fp:
                #     fp.seek(0,0)
                ...
                if tag is True and self.pre_planbatchid == '':
    ...
    def insert_WebLogs(self, List):
        ...
        for j in range(i):
            ...
            sql.append(tempsql)
        tmpsql = "delete from tbl_fk_weblogs where batch_id like '" + batch_id[
                                                                      :-13] + "%'"  # and LENGTH(batch_id)=LENGTH('"+batch_id+"')
    ...
    def update_DataResult(self, flag=True):
        ...
        if flag == False:
            ...
            sql = "select stream_attr from tbl_case_def where  project_id='"+EX.project_id+"' and eparchy_code='"+EX.eparchy_code+"' and case_id='" + case_id + "'"
            # global stream_attr
            stream_attr = self.DB.getSqlFirst(sql)
            # print stream_attr
            if CaseStatus == 'pass' and stream_attr in ('0', '1', '2', '3', '4'):
                # 获取资源
                if stream_attr in ('0', '1'):
                    # 前台(WEB)执行状态,1-成功
                    status = '1'
                    if stream_attr == '0':
                        logging.info("stream_attr:0-正常流，需要获取数据，有后台校验".decode('utf-8').encode('gbk'))
                        # 后台(DB 检查)执行状态,3-待验证
                        verf_status = '3'

                    elif stream_attr == '1':
                        logging.info("stream_attr:1-正常流，需获取数据，无后台校验".decode('utf-8').encode('gbk'))
                        # 后台(DB 检查)执行状态,1-成功
                        verf_status = '1'

                elif stream_attr == '2':
                    logging.info("stream_attr:2-正常流，无需获取数据，有后台校验".decode('utf-8').encode('gbk'))
                    # 前台(WEB)执行状态,1-成功
                    status = '1'
                    # 后台(DB 检查)执行状态,3-待验证
                    verf_status = '3'
                    ...
                    logging.info("stream_attr:3-正常流，无需获取数据，无后台校验".decode('utf-8').encode('gbk'))
                    # 前台(WEB)执行状态,1-成功
                    status = '1'
                    # 后台(DB 检查)执行状态,1-成功
                    verf_status = '1'
                    ...
                    logging.info("stream_attr:4-异常流，需获取数据，无后台校验".decode('utf-8').encode('gbk'))
                    # 前台(WEB)执行状态,1-成功
                    status = '1'
                    # 后台(DB 检查)执行状态,1-成功
                    verf_status = '1'
    ...
    def insert_DataInterface(self, Dict):
        ...
        for keyword_name, keyword_value in Dict.items():
            tmpsql = "insert into tbl_auto_datainterface values('" + batch_id + "','" + case_id + "','" + group_id + "','" + str(
                keyword_name) + "','" + str(
                keyword_value) + "','" + self.group_id + "','" + case_exec_time + "','" + self.project_id + "','" + self.eparchy_code + "')"
            sql.append(tmpsql)
        self.DB.dbwSql(sql)
        Dict = {}
    ...
    def update_Plan_Exec(self, status=2):
        global templan_batch_id
        end_time = self.getCurrentTime()
        sql = "update tbl_plan_exec set status='" + str(
            status) + "',end_time='" + end_time + "' where plan_batch_id='" + templan_batch_id + "'"
        self.DB.dbwSql(sql)
...
def endCase(LineNum, Dict={}, CallInter=0):
    ...
    # if CONFIG["CaseStatus"] == "pass" and stream_attr == '1':
    #     logging.info("case " + str(LineNum) + "属性为异常流，无需后台校验".decode('utf-8').encode('gbk'))
    # elif CONFIG["CaseStatus"] == "pass" and stream_attr == '3':
    #     logging.info("case " + str(LineNum) + "属性为异常流，无需后台校验".decode('utf-8').encode('gbk'))
    # elif CONFIG["CaseStatus"] == "pass" and stream_attr == '4':
    #     logging.info("case " + str(LineNum) + "属性为异常流，无需后台校验".decode('utf-8').encode('gbk'))
...
def contextClick_v(*inputParam):
    ...
    try:
        if innerParam[0][0] == "/":
            logging.log(15, "element disabled status is " + str(
                WD.find_element_by_xpath(innerParam[0]).get_attribute("disabled")))
            ActionChains(WD).context_click(WD.find_element_by_xpath(innerParam[0])).perform()
        else:
            logging.log(15, "element disabled status is " + str(
                WD.find_element_by_css_selector(innerParam[0]).get_attribute("disabled")))
            ActionChains(WD).context_click(WD.find_element_by_css_selector(innerParam[0])).perform()
        return True
    except UnexpectedAlertPresentException:
        WD.switch_to_alert().accept()
        logging.log(15, '这里要重新考虑下元素事件里面会弹出小窗的情况')
...
def inputText_v(*inputParam):
    '''
    支持普通文本输入，同时也支持模拟键盘输入，其中键盘输入为组合输入时，第二个参数为元祖
    普通输入：
    exp1:inputText_v("#id",'hello')
    模拟键盘输入(shift键)：
    exp2:inputText_v('#id','Keys.SHIFT')
    模拟键盘输入(大写N)：
    exp3:inputText_v('#id',(Keys.SHIFT,'n'))
    :param inputParam: css/xpath,text(Keys)
    :return:
    '''
    global WD
    if CONFIG["executeMode"] == 1:
    ...
    try:
        if len(innerParam) == 1:
            ...
            if innerParam[0][0] == "/":
                if type(innerParam[1]) is tuple and innerParam[1][0][:4] == 'Keys.':
                    WD.find_element_by_xpath(innerParam[0]).send_keys(innerParam[1])
                else:
                    WD.find_element_by_xpath(innerParam[0]).send_keys(innerParam[1].decode('utf-8'))
            else:
                if type(innerParam[1]) is tuple and innerParam[1][0][:4] == 'Keys.':
                    WD.find_element_by_css_selector(innerParam[0]).send_keys(innerParam[1])
                else:
...
def inputAlertText_v(input=''):
    ...
    while i < CONFIG['TimeOut']:
        try:
            WD.switch_to_alert().send_keys(input)
            return True
        except NoAlertPresentException:
            i = i + 1
            # return ''
...
def dragAndDrop(srcLoc, dstLoc):
    ...
    try:
        if srcLoc[0] == "/" and dstLoc[0] == "/":
            ...
            logging.log(15,
                        "element disabled status is " + str(WD.find_element_by_xpath(srcLoc).get_attribute("disabled")))
            logging.log(15,
                        "element disabled status is " + str(WD.find_element_by_xpath(dstLoc).get_attribute("disabled")))
            ...
            logging.log(15, "element disabled status is " + str(
                WD.find_element_by_css_selector(srcLoc).get_attribute("disabled")))
            logging.log(15, "element disabled status is " + str(
                WD.find_element_by_css_selector(dstLoc).get_attribute("disabled")))
            ActionChains(WD).drag_and_drop(e, t).perform()
        return True
    except UnexpectedAlertPresentException:
        WD.switch_to_alert().accept()
...
@log
def moveToElement_v(*inputParam):
    global WD
    if CONFIG["executeMode"] == 1:
        innerParam = [] if len(inputParam) == 0 else inputParam[0]
    else:
    ...
    try:
        if innerParam[0][0] == "/":
            logging.log(15, "element disabled status is " + str(
                WD.find_element_by_xpath(innerParam[0]).get_attribute("disabled")))
            move_element_app = WD.find_element_by_xpath(innerParam[0])
        else:
            logging.log(15, "element disabled status is " + str(
                WD.find_element_by_css_selector(innerParam[0]).get_attribute("disabled")))
            move_element_app = WD.find_element_by_css_selector(innerParam[0])
        ActionChains(WD).move_to_element(move_element_app).perform()
        return True
    except UnexpectedAlertPresentException:
        WD.switch_to_alert().accept()
        logging.log(15, '这里要重新考虑下元素事件里面会弹出小窗的情况')
